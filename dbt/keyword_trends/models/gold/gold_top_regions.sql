...
keyword_summary AS (
    SELECT 
        keyword_id,
        keyword,
        MAX(region_name) FILTER (WHERE rank = 1) AS first_most_interested_region,
        MAX(region_name) FILTER (WHERE rank = 2) AS second_most_interested_region, 
        MAX(region_name) FILTER (WHERE rank = 3) AS third_most_interested_region,
